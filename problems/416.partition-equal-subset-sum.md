...
老司机们看到转化后的题，会立马想到背包问题，这里会提供**深搜**和**背包**两种解法。
...
可以看出，可以将原问题转化为规模更小的同样问题，我们考虑使用递归实现。
...
nums[1:n]能否找到和为 target 的子序列，取决于 nums[1:n-1], 如果 nums[1:n-1]能找到和为 target-nums[n]或者 target 的子序列，那么 nums[1:n]肯定也可以。我们用 dp[n][m]来表示前 n 个数能找到和为 m 子序列的可能性，则动态规划方程为：
