...
If we optimize it further, we only need dp[i - 1] and dp[i - 2] when determining each dp[i]. For example, to calculating dp[6], we would only need dp[5] and dp[4], and there's no need to keep dp[3], dp[2], ... in memory.
